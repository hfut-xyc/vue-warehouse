      ...
      <template v-if="step === 0">
        <el-form ref="addProductForm" :model="addProductForm" :rules="rules1" status-icon label-width="120px">
          ...
          <el-form-item label="供货商信息" prop="supplier">
            <el-input type="textarea" v-model="addProductForm.supplier" style="width: 400px"
          ...
          <el-form-item label="操作员工" prop="eid">
            <template v-if="addOrderForm.wid !== ''">
              <el-select v-model="addOrderForm.eid" filterable>
                <el-option v-for="operator in warehouseList.find(item => item.id === addOrderForm.wid).operators"
                  :key="operator.id" :label="operator.name" :value="operator.id">
                </el-option>
              </el-select>
            </template>
            <template v-else>
